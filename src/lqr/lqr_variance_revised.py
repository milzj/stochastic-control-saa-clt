# -*- coding: utf-8 -*-
"""lqr-variance-revised.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1FTlg0oCJTU939cn8pu0fdapPKYgYUXWK
"""
...
def plot_variance_snapshots(params: Dict[str, Any], S_series: np.ndarray, v_series: np.ndarray):
    ...
    ax.set_ylabel(r'Asymptotic Variance $\sigma_t^2(x_t)$')
...
def plot_decomposition_subplots(params: Dict[str, Any], P: np.ndarray, K: np.ndarray, S: np.ndarray, v: np.ndarray):
    ...
    for x_val in x_test_values:
        _, _, total_var = calculate_variance_decomposition(params, P, K, S, v, x_test=x_val)
    ...
    for x_val in x_test_values:
        ...
        local_var, prop_var, total_var = calculate_variance_decomposition(params, P, K, S, v, x_test=x_val)

        ax.plot([], [], ' ', label=f'$x_t = {x_val}$')
        ax.plot(time_axis, total_var, color='black', lw=2.5, linestyle='-', label=r'Total variance $\sigma_t^2(x_t)$')
...
def plot_variance_surfaces(params: Dict[str, Any], P: np.ndarray, K: np.ndarray, S: np.ndarray, v: np.ndarray):
    ...
    _create_surface_plot(Total_C_grid, r'$\sigma_t^2(x_t)$', "variance_surface_total.pdf")
...
if __name__ == "__main__":
    ...
    params = lqr.variance_parameters()
 

    # 2. Run foundational computations once
    print("Computing coefficients...")
    P, K, S, v = compute_coefficients(params)
